...
const dailyMetricsColors = {
  low: demoColors.red,
  mid: demoColors.orange,
  high: demoColors.green
};

const getOeeColor = (val: number): string => {
  if (val < 80) {
    if (val < 40) return dailyMetricsColors.low;
    else return dailyMetricsColors.mid;
  }
  return dailyMetricsColors.high;
};
  ...
  const charts = [
    {
      ...
      color: getOeeColor(Number(data.oee) * 100)
    },
    {
      label: 'Performance',
      progress: Number(data.performance) * 100,
      color: getOeeColor(Number(data.performance) * 100)
    },
    {
      label: 'Availability',
      progress: Number(data.availability) * 100,
      color: getOeeColor(Number(data.availability) * 100)
    }
  ];
