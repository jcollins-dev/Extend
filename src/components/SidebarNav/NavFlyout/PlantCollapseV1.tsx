...
const PlantCollapseV1 = ({ plants, isLoading }: PlantCollapseArgs): React.ReactElement | null => {
  ...
  const highlightColor = navFlyoutHighlight[FleetNavigationEntityType.PLANT] as string;
              ...
              () => {
                if (plantClickable) {
                  if (parseFloat(process.env.REACT_APP_NAV_VERSION as string) >= 2) {
                    history.push(navContext.entityLinks?.plant?.replace(':plantId', p.id) ?? '');
                  } else {
                    history.push(
                      businessUnits.includes('pemea') || businessUnits.includes('pna')
                        ? JBTRoutes.site.replace(':plantId', p.id)
                        : businessUnits.includes('avure')
                        ? `${JBTRoutes.fleet}?plantId=${p.id}`
                        : JBTRoutes.site.replace(':plantId', p.id)
                    );
                  }
...
export default PlantCollapseV1;
