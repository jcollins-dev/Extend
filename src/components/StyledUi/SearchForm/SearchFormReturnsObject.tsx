...
const SuggestedDropdown = ({
  ...
  const Items = suggested?.map((item) => (
    <div className={`${baseClass}__item`} onClick={() => handleClick(item)} key={item}>
      {item}
    </div>
  ));
...
export const SearchForm = ({
  ...
  if (!handleSubmit)
    console.warn(`Error in SearchForm:
    missing handleSubmit function`);
    ...
    if (hasSuggestions) {
      if (suggested) {
        if (suggested?.length == 0) {
          ...
          setSearchTerm(suggested[0]);
          return handleSubmit?.(suggested[0]);
        }
      }
    } else return handleSubmit?.(searchTerm);
  };
