...
import { StackedBarChartOverTimeTooltipContainer } from './StackedBarChartTooltips.elements';
import { format, parseISO } from 'date-fns';

export const StackedBarChartOverTimeTooltip = (props: Record<string, unknown>): JSX.Element => {
  const datum = props?.datum as Record<string, unknown>;
  const className = props?.className as string;

  if (!datum) return <></>;

  const group = datum?.group as string;
