...
export const GetsContainerWidthAndHeight = Template.bind({});
GetsContainerWidthAndHeight.args = {
  title: 'Demo Date Picker Component',
  Load: ({ width, height }: { width?: number; height?: number }): JSX.Element => (
    <div>
      <h4>
        This page consumes with and height of the main content container. Used for charts and
        graphs.
      </h4>
      <div>Resize the page to see updates</div>
      to use: add `Load` property to DashboardWidget props
      <pre>
        {`
<DashboardWidget {...props} 
  Load={
    ({ width, height }: ({ width?: number, height?: number })): JSX.Element => {
      ...
      cotainer width: {width} - height: {height}
    </div>
  ),
  linksToPath: 'go/to/path'
};
