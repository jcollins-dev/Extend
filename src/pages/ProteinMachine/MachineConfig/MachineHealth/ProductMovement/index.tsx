...
const MachineProductMovement = ({
  ...
  const {
    ...
    labels: [MachineHealthTabs.ProductMovement],
    languageId: languageId,
    showInactive: true
  });

  const [updateProductMovementData, { isLoading: updating, error: errorUpdate }] =
    useUpdateMachineConfiguratorDataMutation();
  ...
  const saveInfo = async () => {
    ...
    const updatedProductMovementTab = {
      // Copy the root of the data object
      ...tabData,
      active: isActive,
      // Overwrite the members from local state
    ...
    await updateProductMovementData({
      machineId,
      widget: updatedProductMovementTab,
      languageId: languageId
    })
      .unwrap()
      .then(() => {
        toast.success(`Product movement updated successfully`);
        setIsDirty(false);
      })
      .catch((error) => {
        toast.error(`Failed to update product movement`);
    ...
    if ((!copyData || !copyData?.[0]) && isCopying && !copyModalOpen && !copyFetching) {
      ...
      setTabData(
        nextCopiedConfiguration
          ...
          ?.members?.find((member) => member.label === MachineHealthTabs.ProductMovement)
      );
...
export default MachineProductMovement;
