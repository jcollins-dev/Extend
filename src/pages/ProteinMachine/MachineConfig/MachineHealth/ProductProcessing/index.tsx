...
const MachineProductProcessing = ({
  copyWidgetType,
  handleCopyCallback,
  hasWidgets,
  hideButtonRow,
  ...
  const {
    ...
    labels: [MachineHealthTabs.ProductProcessing],
    languageId: languageId,
    showInactive: true
  });

  const [updateProductProcessingData, { isLoading: updating, error: errorUpdate }] =
    useUpdateMachineConfiguratorDataMutation();
  ...
  const saveInfo = async () => {
    ...
    await updateProductProcessingData({
      ...
      .unwrap()
      .then(() => {
        toast.success(`Product processing updated successfully`);
      })
      .catch((error) => {
        toast.error(`Failed to update product processing`);
    ...
    if ((!copyData || !copyData?.[0]) && isCopying && !copyModalOpen && !copyFetching) {
      ...
      setTabData(
        nextCopiedConfiguration
          ...
          ?.members?.find((member) => member.label === MachineHealthTabs.ProductProcessing)
      );
...
export default MachineProductProcessing;
